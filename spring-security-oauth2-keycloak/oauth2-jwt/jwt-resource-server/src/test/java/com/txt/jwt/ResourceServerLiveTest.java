...
public class ResourceServerLiveTest {
    ...
    private final String authorizeUrlPattern = "http://localhost:8083/auth/realms/syspro/protocol/openid-connect/auth?response_type=code&client_id=jwtClient&scope=%s&redirect_uri=" + redirectUrl;
    private final String tokenUrl = "http://localhost:8083/auth/realms/syspro/protocol/openid-connect/token";
    private final String resourceUrl = "http://localhost:8081/jwt-resource-server/api/foos";
    private final String userInfoResourceUrl = "http://localhost:8081/jwt-resource-server/user/info";
    ...
    public void givenUserWithReadScope_whenGetUserInformationResource_thenSuccess() {
        ...
        assertThat(response.as(Map.class)).containsEntry("user_name", "john@test.com")
                .containsEntry("organization", "SYSPRO");
    }
    ...
    public void givenUserWithReadScope_whenPostNewProjectResource_thenForbidden() {
        String accessToken = obtainAccessToken("read");
        System.out.println("ACCESS TOKEN: " + accessToken);
        FooDTO newFoo = new FooDTO(0, "newFoo");
